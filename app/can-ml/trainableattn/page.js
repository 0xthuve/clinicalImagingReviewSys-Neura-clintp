...
export default function CancerDetectPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      console.log("Sending request to:", `${API_BASE_URL}/trainableattn`);
      const response = await axios.post(`${API_BASE_URL}/trainableattn`, formData, {
    ...
    <div className="min-h-screen bg-gray-100 py-10 px-4">
      <div className="max-w-xl mx-auto bg-white p-6 rounded-2xl shadow-lg">
        <h1 className="text-2xl font-bold text-center text-pink-600 mb-4">
          🧠 Breast Cancer Prediction with Attention Map
        </h1>
  ...
  <div className="mt-4 space-y-4">
    <div>
      <p className="text-sm text-gray-500 mb-2">Trainable Attention Heatmap:</p>
      <img
        src={`data:image/png;base64,${result.images_base64.heatmap}`}
        alt="Trainable Attention Heatmap"
        className="rounded-lg w-full"
      />
    </div>
    <div>
      <p className="text-sm text-gray-500 mb-2">Trainable Attention Overlay:</p>
      <img
        src={`data:image/png;base64,${result.images_base64.overlay}`}
        alt="Trainable Attention Overlay"
        className="rounded-lg w-full"
      />
    </div>
    <div>
      <p className="text-sm text-gray-500 mb-2">Trainable Attention Full:</p>
      <img
        src={`data:image/png;base64,${result.images_base64.full}`}
        alt="Trainable Attention Full"
        className="rounded-lg w-full"
      />
    </div>
  </div>
